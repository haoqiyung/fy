...
# 创建日志文件路径
log_file = os.path.join(log_dir, "false.txt")

# 创建logger对象
logger = logging.getLogger("false_logger")
...
def translate_text(source_text, source_lang, target_lang):
    ...
    while retry_count < MAX_RETRY_COUNT:
        try:
            ...
            create_connection()
            retry_count += 1
    return "所有翻译 API 均不可用，请检查网络连接是否正常或者尝试切换 API。"
...
def show_about():
    ...
    if about_window is None or not about_window.winfo_exists():
        ...
        style = ttk.Style()
        style.configure("About.TLabel", font=("Arial", 10), padding=10)
        style.configure("About.TButton", font=("Arial", 8), padding=5)

        about_label = ttk.Label(about_window, text="AI 翻译助手\n\n作者: Hao7\n\nGithub: https://github.com/Haoqi7\n\n博客: https://haoqiyun.tk", style="About.TLabel")
        about_label.pack()

        ok_button = ttk.Button(about_window, text="确定", style="About.TButton", command=about_window.destroy)
...
title_label = tk.Label(header_frame, text="AI 翻译助手", font=("Arial", 15))
...
export_button = tk.Button(history_frame, text="导出历史记录", command=export_history, width=15)
export_button.grid(row=0, column=1, padx=(0, 5), pady=5)

clear_history_button = tk.Button(history_frame, text="清空历史记录", command=clear_history, width=15)
clear_history_button.grid(row=0, column=2, padx=(5, 0), pady=5)

history_listbox = tk.Listbox(history_frame, height=10, width=50)
history_listbox.grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky="nsew")

history_scrollbar = tk.Scrollbar(history_frame, orient="vertical")
history_scrollbar.grid(row=1, column=3, padx=5, pady=5, sticky="ns")
