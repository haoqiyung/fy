...
TRANSLATE_API_URLS = config.get('API', 'TRANSLATE_URLS').split(',')

conn = None

def get_connection():
    global conn
    if conn is None or not conn.sock:
        TRANSLATE_API_URL = random.choice(TRANSLATE_API_URLS)
        conn = http.client.HTTPSConnection(TRANSLATE_API_URL)
    return conn
