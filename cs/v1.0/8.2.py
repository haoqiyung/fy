...
class Translator(tk.Frame): 
    def __init__(self, master=None):
        ...
        self.history = []
        self.target_lang = tk.StringVar(value="中文")  # 添加这行代码
        self.create_widgets()
        self.create_menu()
    ...
    def create_menu(self):
        ...
        for i, api in enumerate(API_ENDPOINTS):
            api_menu.add_radiobutton(label=f"API{i+1}", variable=self.api_index, value=i)
        menu_bar.add_cascade(label="API优先级", menu=api_menu)
        ...
        # 清空输入输出框
        menu_bar.add_command(label="清空", command=self.clear_text)
