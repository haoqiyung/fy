...
class Translator(tk.Frame):
    ...
    def create_widgets(self):
        self.input_label = tk.Label(self, text="要翻译的文本:", font=('Arial', 12), bg='#F9F9F9')
        self.input_label.pack()
        self.input_text = tk.Text(self, height=10, width=50, font=('Arial', 12), bg='#FFFFFF', fg='#333333', wrap='word')
        self.input_text.pack(fill='both', expand=True, padx=5, pady=5)
        self.input_scrollbar = ttk.Scrollbar(self, orient='vertical', command=self.input_text.yview)
        self.input_scrollbar.pack(side='right', fill='y')
        self.input_text.config(yscrollcommand=self.input_scrollbar.set)

        self.lang_frame = tk.Frame(self, bg='#F9F9F9')
        ...
        self.target_lang_input.pack(side='left', padx=5)

        self.translate_button = tk.Button(self, text="翻译", font=('Arial', 12), command=self.translate)
        self.translate_button.pack(pady=5)

        self.output_label = tk.Label(self, text="翻译结果:", font=('Arial', 12), bg='#F9F9F9')
        self.output_label.pack()
        self.output_text = tk.Text(self, height=10, width=50, font=('Arial', 12), bg='#FFFFFF', fg='#333333', wrap='word')
        self.output_text.pack(fill='both', expand=True, padx=5, pady=5)
        self.output_scrollbar = ttk.Scrollbar(self, orient='vertical', command=self.output_text.yview)
        self.output_scrollbar.pack(side='right', fill='y')
        self.output_text.config(yscrollcommand=self.output_scrollbar.set)

        self.quit_button = tk.Button(self, text="退出", font=('Arial', 12), command=self.master.destroy)
        self.quit_button.pack(pady=5)

        self.api_index = 0
        self.api_label = tk.Label(self, text=f"当前使用的 API: {API_ENDPOINTS[self.api_index]}", font=('Arial', 10), bg='#F9F9F9')
        self.api_label.pack(pady=5)
        self.api_button = tk.Button(self, text="切换 API", font=('Arial', 10), command=self.switch_api)
